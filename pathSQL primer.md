...
pathSQL is the name of a dialect of SQL defined for ChaosDB. Although the [ChaosDB data model](./terminology.md#essential-concepts-data-model) is different
...
How to insert data
------------------
There is no concept of table in ChaosDB: all data are inserted in global scope. Two styles can be used to
perform [PIN](./terminology.md#pin) inserts:

1. ChaosDB-specific syntax:
...
Collections may contain heterogeneous data, e.g.
  ...
  <code class='pathsql_snippet'>INSERT prop_collection = {'string', X'DEF5', U'http://test/', 128, 3.40282f, 3.40282, true, TIMESTAMP '2010-12-31 23:59:59', INTERVAL '-12:00:00'};</code>  

Whole graphs may be inserted in one statement, e.g.

  <code class='pathsql_snippet'>INSERT (name, bornin, email, livesin, age, friends, photos) VALUES
...
More examples:
  ...
  <code class='pathsql_snippet'>INSERT "http://acme.org/properties/length"=123, "http://acme.org/properties/width"=456, "http://acme.org/properties/name"='wonderful instrument';</code>  

  <code class='pathsql_snippet'>PREFIX acmep: 'http://acme.org/properties/' INSERT acmep:"length"=123.1, acmep:width=456.1, acmep:name='great instrument';</code>  
...
How to query 
------------
Most of the querying syntax in ChaosDB is compatible with standard SQL: expressions, function calls, WHERE, ORDER BY,
...
How to use joins
----------------
ChaosDB returns immutable PIN collections as query results. Here's an example:
...
-----------------------
Relational databases use foreign keys to establish relationships between tables.  ChaosDB offers a powerful
alternative with [references](./terminology.md#pin-reference) (similar to object-oriented databases):

  <code class='pathsql_snippet'>UPDATE ADD friends=(SELECT ks:pinID WHERE name='Fred') WHERE name='Jurgen';</code>  
...
-----------------------
Instead of simply allowing to create indexes ("CREATE INDEX" statement of relational databases), ChaosDB
